"""
bot/mod/music/embeds.py

Modification():

- 提供 Music Module 的 Embed 工廠函式。
- 提供正在播放、佇列、歷史與播放清單顯示。
- 提供音樂功能面板與通用成功、錯誤、資訊 Embed。
- 從 Music 設定讀取 Embed 顏色、循環標籤與 Footer。

本檔只負責 Music Module 的 Embed 呈現，不處理播放器業務邏輯。
"""
...
from bot.mod.music.queue import MusicQueue, LoopMode
from bot.mod.music.song  import Song, format_duration
...
def music_panel_embed(player) -> discord.Embed:
    """建立音樂功能面板 Embed。"""

    current = player.current_song
    status = (
        f"正在播放：**{current.title}**"
        if current
        else "目前沒有播放中的音樂"
    )

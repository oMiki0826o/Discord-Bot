...
- 從 cogs/utility/favorites.py 移至 cogs/voice/favorites.py，讓音樂收藏與語音/播放功能放在同一類別。
...
from bot.mod.music.formatter import format_duration

logger = logging.getLogger("bot.voice.favorites")
...
def _fav_embed(
    ...
    if not favorites:
        ...
        embed.description = "\n".join(lines)
        embed.set_footer(
            text=f"第 {page}/{total_pages} 頁，共 {total} 首  |  {get('embed_footer.default','Discord Bot')}"
        )
    return embed
...
class FavoriteListView(discord.ui.View):
    ...
    async def play_selected(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        from bot.mod.music.embeds import error_embed, info_embed
...
class Favorites(commands.Cog):
    ...
    async def add_by_query(
        ...
        """
        from bot.mod.music.embeds import error_embed, info_embed, success_embed
    ...
    async def play_favorite_core(
        ...
        """
        from bot.mod.music.embeds import error_embed, now_playing_embed, success_embed
        ...
        private_confirmation = success_embed(f"已加入待播清單：**{song.title}**")
        if was_active:
            return private_confirmation, MusicControls(player)
